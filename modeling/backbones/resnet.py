...
from modeling.metrics.metrics2d import JointsMSELoss, KeypointsMSESmoothLoss, MaskedMSELoss, compute_stage_loss, evaluate_normalized_mean_error, calculate_err, calc_pck, JDR
...
class PoseResNet(nn.Module):
    ...
    def __init__(self, block, layers, cfg, **kwargs):
        ...
        self.new_up_layer = self._make_new_up_layer(
            NUM_DECONV_LAYERS,
            r = 2
        )
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        ...
        return nn.Sequential(*layers)
    def _make_new_up_layer(self, num_layers, r):
        layers = []
        self.up_layer = nn.Sequential(
            nn.Conv2d(in_channels=2048, out_channels=2048 * 2 * 2, kernel_size=1, stride=1, padding=0,
                      bias=False),
            nn.PixelShuffle(r),
           # nn.BatchNorm2d(2048, momentum=self.BN_MOMENTUM),
            nn.ReLU(inplace=True)
        )
        for i in range(num_layers):
            layers.append(self.up_layer)

        layers.append(
            nn.Conv2d(in_channels=2048, out_channels=256, kernel_size=1, stride=1, padding=0,
                      bias=False)
        )
        layers.append(nn.BatchNorm2d(256, momentum=self.BN_MOMENTUM))
    ...
    def forward(self, x,other_inputs=[None, None, None, None, None, None, None],keypoint_vis = None,  other_heatmaps_loss = None):
        batch_size = x.shape[0]
        weights = torch.tensor(1)
        other_features, other_KRT, other_heatmaps, KRT, camera, other_camera, other_preheatmaps = other_inputs
        features, heatmaps, batch_locs, batch_scos, corr_poss, depths,preheatmaps = [], [], [], [], [], [], []
        if  other_heatmaps_loss != None and other_heatmaps != None:
           # print('resnet')
           # print(other_preheatmaps.shape)
          #  print(other_heatmaps.shape)
            other_loss_dict = self.criterion(other_heatmaps_loss[0], other_heatmaps, keypoint_vis)
            weights = weights.to(other_loss_dict)
            weights = min(1, 0.0015 / other_loss_dict)
            # print(weights)
        # 3 x 256 x 256
       # print('x的shape'.format(x.shape))
        ...
        def getOtherFeat(feat,preheatmaps = None, tar_heatmaps= None,sampler=None):
            ...
            if 'epipolarpose' in cfg.BACKBONE.BODY:
                ret, corr_pos, depth, sample_locs = \
                    sampler(feat, other_features, KRT, other_KRT,preheatmaps,tar_heatmaps ,\
                        camera=camera, other_camera=other_camera)
                if weights != None:
                   # print(weights)
                    return weights*ret + feat, corr_pos, depth, sample_locs
                else:
                    return ret + feat, corr_pos, depth, sample_locs
            
        if cfg.EPIPOLAR.MERGE == 'early':
            feature = x
            x, corr_pos, depth, sample_locs = getOtherFeat(feature,other_preheatmaps,other_heatmaps, sampler=self.epipolar_sampler)
            depths.append(depth)
            corr_poss.append(corr_pos)
        elif cfg.EPIPOLAR.MERGE == 'both':
            feature = x
            x, _, _, _ = getOtherFeat(feature,other_preheatmaps,other_heatmaps, sampler=self.epipolar_sampler)
        ...
        if cfg.EPIPOLAR.MERGE == 'late':
            ...
            x, corr_pos, depth, sample_locs = getOtherFeat(feature,other_preheatmaps,other_heatmaps, sampler=self.epipolar_sampler1)       
            depths.append(depth)
            corr_poss.append(corr_pos)                 
        else:
            x = feature
        ...
        #20 x 64 x 64
        preheatmaps = self.final_layer(x)
        ...
        return feature, heatmaps, batch_locs, batch_scos, corr_poss, depths, sample_locs,preheatmaps 
