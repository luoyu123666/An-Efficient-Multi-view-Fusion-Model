...
class Modelbuilder(nn.Module):
    ...
    def forward(self, inputs, is_train=True):
        ...
        other_heatmaps = inputs.get('other_heatmap')
        if other_heatmaps is not None:
            other_heatmaps = other_heatmaps.to(torch.float32)
            ...
            if cfg.DATASETS.TASK == 'multiview_keypoint':
                if cfg.EPIPOLAR.MULTITEST:
                    ...
                    with ExitStack() as stack:
                        ...
                        other_features, other_heatmaps_loss, _, _,_, _, _, other_preheatmaps = self.backbone(other_img)
                        # if other_preheatmaps != None and other_heatmaps != None:
                        #      other_loss_dict = self.criterion(other_preheatmaps[0], other_heatmaps,keypoint_vis)
                    ...
                    _, batch_heatmaps, batch_locs, batch_scos, corr_pos, depths, sample_locs, warpedheatmap = \
                        self.reference(img,[other_features, other_KRT, other_heatmaps, KRT, camera, other_camera, other_preheatmaps],keypoint_vis, other_heatmaps_loss)
            elif cfg.DATASETS.TASK == 'keypoint':
