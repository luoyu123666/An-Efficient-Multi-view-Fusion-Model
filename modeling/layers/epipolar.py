...
class Epipolar(nn.Module):
    ...
    def forward(self, feat1, feat2, P1, P2, preheatmaps,tar_heatmaps,depth=None, camera=None, other_camera=None, ref1=None, ref2=None):
        """ 
        Args:
            feat1         : N x C x H x W
            feat2         : N x C x H x W
        ...
        for i in range(N):
            ...
            if usedepth:
                ...
                if cfg.EPIPOLAR.PRIOR:
                    ...
                    other_sim = self.epipolar_similarity(preheatmaps[i], tar_heatmaps[i],
                                                   int(other_camera[i].item()), int(other_camera[i].item()))
                    ...
                    other_sim = self.epipolar_similarity(preheatmaps[i], tar_heatmaps[i])
                # print(type(sim))
                #print(type(other_sim))
                # print(sim.shape)
                # print(sim.view(-1, 1, H, W).shape)
                # print(other_sim.shape)
                # print(other_sim.view(-1, 1, H, W).shape)
            if cfg.EPIPOLAR.ATTENTION == 'max':
                ...
                with torch.no_grad():
                    ...
                    pos = de_normalize(pos, H, W)
                    corr_pos.append(pos)
                if sim.shape == other_sim.shape:
                    tmp = (other2_sampled * (sim.view(-1, 1, H, W)) * other_sim.view(-1, 1, H, W)).sum(0)
                   # print('小可爱')
                else:                  
    ...
    def epipolar_similarity(self, feat1, sampled_feat2, cam1=None, cam2=None):
        ...
        """
        C, H, W = feat1.shape
        if len(sampled_feat2.shape)!= 4:
            sample_size = cfg.EPIPOLAR.SAMPLESIZE
            sampled_feat2 = sampled_feat2.view(1, C, H, W).expand(sample_size, -1, -1, -1)
        else:
            sample_size = sampled_feat2.shape[0]
            
       # print(feat1.shape, sampled_feat2.shape)
        # if sample_size == cfg.EPIPOLAR.SAMPLESIZE:
        #     sample_size = sampled_feat2.shape[0]
        # else:
        #     sample_size = cfg.EPIPOLAR.SAMPLESIZE
        #     sampled_feat2 = sampled_feat2.view(1, C, H, W).expand(sample_size, -1, -1, -1)
        ...
        if sim.shape[0]  != 64:
            for i in range(4):
               ...
               # print('第i:{}次的shape：{}'.format(i, sim.shape))
        return sim
